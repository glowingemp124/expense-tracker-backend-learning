...
const userSchema = new mongoose.Schema(
  {
    ...
    provider: {
      ...
      type: String,
      enum: ["google", "facebook", "apple", "email", "guest"], // Provider types
      default: "email",
    },
    googleId: {
      type: String,
...
userSchema.statics.findByCredentials = async (email, password, userType) => {
  ...
  if (userType && user.accountState.userType !== userType) {
    return { error: "user_type_mismatch" }; // Return an error key if user type doesn't match
  }
